...
> This snapshot contains the project structure, pinned dependencies, test
> harness, configuration templates, and the architectural decision log. The
> feature modules are stubs at this point; each is implemented in a later
> phase (see [Roadmap](#roadmap)). `streamlit run app.py` currently launches a
> placeholder page. The running decision log in [`DECISIONS.md`](DECISIONS.md)
> is the source of truth for *why* each choice was made.
...
| --- | --- | --- |
| 1 | Project scaffold, deps, test harness, config, decision log | ✅ this repo |
| 2 | Webcam interface (`streamlit-webrtc`) | stub |
